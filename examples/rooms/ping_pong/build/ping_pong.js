...
var INITIAL_MEMORY = Module["INITIAL_MEMORY"] || 31457280;
...
var PThread = {
 ...
 allocateUnusedWorker: function() {
  var pthreadMainJs = locateFile("ping_pong.worker.js");
...
function ___js_game_client_dequeue_packet(lenPtr) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(1, 1, lenPtr);
...
function ___js_game_client_init(roomIdPtr) {
 let nbnet;
...
function ___js_game_client_send_packet(packetPtr, packetSize) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(2, 1, packetPtr, packetSize);
 const data = new Uint8Array(Module.HEAPU8.subarray(packetPtr, packetPtr + packetSize));
 this.gameClient.send(data);
}

function ___js_game_client_start(hostPtr, port) {
...
function ___js_game_server_close_client_peer(peerId) {
 this.gameServer.closePeer(peerId);
}

function ___js_game_server_dequeue_packet(peerIdPtr, lenPtr) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(3, 1, peerIdPtr, lenPtr);
...
function ___js_game_server_init(hostPtr, port) {
 let nbnet;
 if (typeof window === "undefined") {
  nbnet = require("nbnet");
 } else {
...
function ___js_game_server_send_packet_to(packetPtr, packetSize, peerId) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(4, 1, packetPtr, packetSize, peerId);
 const data = new Uint8Array(Module.HEAPU8.subarray(packetPtr, packetPtr + packetSize));
 this.gameServer.send(data, peerId);
}

function ___js_game_server_start() {
 return Asyncify.handleSleep(function(wakeUp) {
...
var Asyncify = {
 ...
 instrumentWasmImports: function(imports) {
  var ASYNCIFY_IMPORTS = [ "env.__js_game_server_start", "env.__js_game_client_start", "env.__js_game_client_close", "env.invoke_*", "env.__call_main", "env.emscripten_sleep", "env.emscripten_wget", "env.emscripten_wget_data", "env.emscripten_idb_load", "env.emscripten_idb_store", "env.emscripten_idb_delete", "env.emscripten_idb_exists", "env.emscripten_idb_load_blob", "env.emscripten_idb_store_blob", "env.SDL_Delay", "env.emscripten_scan_registers", "env.emscripten_lazy_load_code", "env.emscripten_fiber_swap", "wasi_snapshot_preview1.fd_sync", "env.__wasi_fd_sync", "env._emval_await", "env.dlopen" ].map(function(x) {
   return x.split(".")[1];
  });
...
var proxiedFunctionTable = [ null, ___js_game_client_dequeue_packet, ___js_game_client_send_packet, ___js_game_server_dequeue_packet, ___js_game_server_send_packet_to, _tzset, ___sys_fcntl64, ___sys_ioctl, ___sys_open, _abort, _emscripten_set_canvas_element_size_main_thread, _fd_close, _fd_read, _fd_seek, _fd_write ];
...
var asmLibraryArg = {
 ...
 "__js_game_client_dequeue_packet": ___js_game_client_dequeue_packet,
 "__js_game_client_init": ___js_game_client_init,
 "__js_game_client_send_packet": ___js_game_client_send_packet,
 "__js_game_client_start": ___js_game_client_start,
 "__js_game_server_close_client_peer": ___js_game_server_close_client_peer,
 "__js_game_server_dequeue_packet": ___js_game_server_dequeue_packet,
 "__js_game_server_init": ___js_game_server_init,
 "__js_game_server_send_packet_to": ___js_game_server_send_packet_to,
 "__js_game_server_start": ___js_game_server_start,
...
var __emscripten_allow_main_runtime_queued_calls = Module["__emscripten_allow_main_runtime_queued_calls"] = 9016;

var __emscripten_main_thread_futex = Module["__emscripten_main_thread_futex"] = 73740;
