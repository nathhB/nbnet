...
struct __NBN_Channel
{
    ...
    void (*OnOutgoingMessageAcked)(NBN_Channel *, uint16_t);
};
        ...
        NBN_Connection *connection, NBN_Channel *channel, unsigned int message_size)
{
    ...
    for (unsigned int i = 0; i < chunk_count; i++)
    {
        ...
        NBN_LogTrace("Enqueue chunk %d (size: %d) for message %d of type %d",
                i, chunk_size, msg_header.id, msg_header.type);
...
NBN_UnreliableOrderedChannel *NBN_UnreliableOrderedChannel_Create(uint8_t id)
{
    ...
    channel->base.read_chunk_buffer = NBN_MemoryManager_Alloc(NBN_CHANNEL_RW_CHUNK_BUFFER_INITIAL_SIZE);
    channel->base.write_chunk_buffer = NBN_MemoryManager_Alloc(NBN_CHANNEL_RW_CHUNK_BUFFER_INITIAL_SIZE);
    channel->base.read_chunk_buffer_size = NBN_CHANNEL_RW_CHUNK_BUFFER_INITIAL_SIZE;
    channel->base.write_chunk_buffer_size = NBN_CHANNEL_RW_CHUNK_BUFFER_INITIAL_SIZE;
    channel->base.next_outgoing_message_id = 0;
    channel->base.next_recv_message_id = 0;
    ...
    {
        channel->base.recved_message_buffer[i].free = true;
        channel->base.outgoing_message_buffer[i].free = true;
    }
...
static void NBN_ReliableOrderedChannel_OnOutgoingMessageAcked(NBN_Channel *, uint16_t);
...
void NBN_EventQueue_Enqueue(NBN_EventQueue *events_queue, int type, void *data)
{
    ...
    if (data)
    {
        if (type == NBN_MESSAGE_RECEIVED || type == NBN_CLIENT_MESSAGE_RECEIVED)
            memcpy(&ev->data.message, data, sizeof(NBN_Message));
        else if (type == NBN_NEW_CONNECTION || type == NBN_CLIENT_DISCONNECTED)
            ev->data.connection = data;
    }
...
void* NBN_Endpoint_CreateOutgoingMessage(NBN_Endpoint *endpoint, uint8_t msg_type, uint8_t channel_id)
{
    ...
    NBN_MessageDestructor msg_destructor = endpoint->message_destructors[msg_type];
    ...
    endpoint->outgoing_message.destructor = msg_destructor;
...
static int NBN_GameClient_ProcessReceivedMessage(NBN_Message *message, NBN_Connection *server_connection)
{
    ...
    if (message->header.type == NBN_MESSAGE_CHUNK_TYPE)
    {
        ...
        if (NBN_Channel_AddChunk(channel, message))
        {
            ...
            complete_message.sender = server_connection;

            NBN_EventQueue_Enqueue(__game_client.endpoint.events_queue, NBN_MESSAGE_RECEIVED, &complete_message);
        }
    }
    else
    {
        message->sender = server_connection;
...
static int NBN_GameClient_HandleMessageReceivedEvent(void)
{
    NBN_Message *message = &__game_client.endpoint.events_queue->last_event_data.message;
...
static int NBN_GameServer_ProcessReceivedMessage(NBN_Message *message, NBN_Connection *client)
{
    ...
    if (message->header.type == NBN_MESSAGE_CHUNK_TYPE)
    {
        ...
        if (NBN_Channel_AddChunk(channel, message))
        {
            ...
            complete_message.sender = client;

            NBN_EventQueue_Enqueue(__game_server.endpoint.events_queue, NBN_CLIENT_MESSAGE_RECEIVED, &complete_message);
        }
    }
    else
    {
...
static void NBN_GameServer_HandleClientConnectionEvent(void)
{
    server_last_event_client = __game_server.endpoint.events_queue->last_event_data.connection;
}
...
static int NBN_GameServer_HandleMessageReceivedEvent(void)
{
    NBN_Message *message = &__game_server.endpoint.events_queue->last_event_data.message;
